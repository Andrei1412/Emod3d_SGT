...
#            print(source)
#            
#            for depth_id in range(-1,2):
#                source[2] = S[ishot-1,2] + depth_id*2 # adjust idepth for dh=2km
#    #            print(coord_sgt)
...
#    #            gyz=Myz[i_source,:]
#    
#                gxx = time_shift_emod3d(Mxx[i_source,:],delay_Time,dt)
#                gyy = time_shift_emod3d(Myy[i_source,:],delay_Time,dt)
#                gzz = time_shift_emod3d(Mzz[i_source,:],delay_Time,dt)
#    #
...
#                gxy = 0.5*time_shift_emod3d(Mxy[i_source,:],delay_Time,dt)
#                gxz = 0.5*time_shift_emod3d(Mxz[i_source,:],delay_Time,dt)
#                gyz = 0.5*time_shift_emod3d(Myz[i_source,:],delay_Time,dt)
#                
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,0] = butter_bandpass_filter(gxx, lowcut, highcut, fs, order=4)
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,4] = butter_bandpass_filter(gyy, lowcut, highcut, fs, order=4)
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,8] = butter_bandpass_filter(gzz, lowcut, highcut, fs, order=4)
#    
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,1] = butter_bandpass_filter(gxy, lowcut, highcut, fs, order=4)
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,3] = AA[ishot_id*3+int(depth_id)+1,i,k,:,1]
#    
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,2] = butter_bandpass_filter(gxz, lowcut, highcut, fs, order=4)
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,6] = AA[ishot_id*3+int(depth_id)+1,i,k,:,2]           
#    
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,5] = butter_bandpass_filter(gyz, lowcut, highcut, fs, order=4)
#                AA[ishot_id*3+int(depth_id)+1,i,k,:,7] = AA[ishot_id*3+int(depth_id)+1,i,k,:,5]
#
##print(AA)                                          
#np.savetxt('AA_SGT.txt',AA.reshape(-1))
AA_arr = np.loadtxt('AA_SGT.txt')
AA=AA_arr.reshape([3*Ns,Nr,3,nts,9])
nt=nts
#input('-->')
##############################################
...
#    mainfolder_o='../../Kernels/Vel_ob_new_V2/Vel_ob_'+str(ishot)+'/'
    ...
    for i,statname in enumerate(statnames):
        if(np.sum(R_all[i,:,ishot-1]==3)):
            s0=statname+GV[0]
            try:
                gmdata_obs_vi_O = timeseries.read_ascii(mainfolder_o+s0)
                statnames_shot_i.append(statname)
            except:
                continue     
    
#    obs=np.zeros(int(3*Nr*nt))
#    index_obs=np.zeros(int(3*Nr*nt))    
    #print(3*Nr*nt)

    print(statnames_shot_i)    
    Nri = len(statnames_shot_i)
    print('Nri='+str(Nri))
    
    obsi=np.zeros(int(3*Nri*nt))
    syni=np.zeros(int(3*Nri*nt))    
    #index_obsi=np.zeros(int(3*Nri*nt))     
    index_obsi=np.ones(int(3*Nri*nt))

#    for ik,statname in enumerate(statnames_RGT):
#        if((statname in statnames_shot_i)):
#            i = statnames_shot_i.index(statname)
    
    for i,statname in enumerate(statnames_shot_i):    
            print(statname+' index:'+str(i))
    ...
    #            if ((distance<200) and (distance>0) and (R_all[i,k,ishot-1]==1)):
                ...
                #stat_data_0_O = stat_data_0_S   #check with synthetic data 
                ...
                #Fill vector index for the whole time series
                index_obsi[nt*3*i+nt*k:nt*3*i+nt*(k+1)]=wd
                #Velocity seismogram only:
                stat_data_0_S = np.multiply(stat_data_0_S,wd)
                stat_data_0_O = np.multiply(stat_data_0_O,wd)                 
                
                print(nt*3*i+nt*k)
                if(k>0):
                    obsi[nt*3*i+nt*k:nt*3*i+nt*(k+1)] = -stat_data_0_O
                    syni[nt*3*i+nt*k:nt*3*i+nt*(k+1)] = -stat_data_0_S                    
                else:
                    obsi[nt*3*i+nt*k:nt*3*i+nt*(k+1)] = stat_data_0_O
                    syni[nt*3*i+nt*k:nt*3*i+nt*(k+1)] = stat_data_0_S
...
#    np.savetxt('AA_ishot.txt',AA_ishot.reshape(-1))
    np.savetxt('obs.txt',obsi.reshape(-1))    
    np.savetxt('syn.txt',syni.reshape(-1))    
#    input('-->')                                        
    #Add iso(M0)=0 to A and d
#    d=np.r_[d, 0]
    obs0 = obsi   
    syn0 = syni 
    obsi=np.r_[obsi,[0,0,0,0]]
    syni=np.r_[syni,[0,0,0,0]]    
    index_obsi=np.r_[index_obsi,[0,0,0,0]] 
    
    misfit_org = np.sum(np.multiply(np.square((syni-obsi)),index_obsi)) 
    print('misfit_org='+str(misfit_org))
...
#        AA_ishot = AA[ishot_id*3+int(depth_id)+1,:,:,:,:]        
#        AA_ishot=np.reshape(AA_ishot,[Nr*3*nt,9])
        
        AA_ishot=np.zeros([Nri,3,nt,9])
        for i,statname in enumerate(statnames_shot_i):    
            if((statname in statnames_RGT)):
                ik = statnames_RGT.index(statname)    
                AA_ishot[i,:,:,:] =  AA[ishot_id*3+int(depth_id)+1,ik,:,:,:]
            
        AA_ishot = AA_ishot.reshape([Nri*3*nt,9])
        
        np.savetxt('AA_ishot.txt',AA_ishot.reshape(-1))
        sio.savemat('Ad_new.mat', {'d': obs0, 'AA0': AA_ishot})
        print(ishot_id*3+int(depth_id)+1)
        input('-->')
    ...
    #    d=np.r_[d, 0]; d=np.r_[d, 0]; d=np.r_[d, 0];
        ...
        AA_d=np.matmul(np.transpose(AA_ishot),obsi)
        M=np.matmul(AA_inv,AA_d)
        #Reconstruct synthetic for misfit calculation:
        syn = np.matmul(AA_ishot,M) 
        syn_arr = syn.reshape(-1)
        misfit_new = np.sum(np.multiply(np.square((syn_arr-obsi)),index_obsi))      
...
np.savetxt('CMT_inv_13events_REAL.txt',M_all_array)

print(idepth_all)    
np.savetxt('Idepth_inv_13events_REAL.txt',idepth_all.reshape(-1))
