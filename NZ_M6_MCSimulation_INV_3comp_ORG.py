...
#    AA6[i,k,:,3] = 0.5*AA_ishot[i,k,:,1]; AA6[i,k,:,4] = 0.5*AA_ishot[i,k,:,2]; AA6[i,k,:,5] = 0.5*AA_ishot[i,k,:,5];
    #AA6[i,k,:,3] = 2*AA_ishot[i,k,:,1]; AA6[i,k,:,4] = 2*AA_ishot[i,k,:,2]; AA6[i,k,:,5] = 2*AA_ishot[i,k,:,5];
    
    
    d_org = np.array(d[i,k,:])
    #d_rec = np.array(np.sum(np.multiply(AA_ishot[i,k,:,0:9AA_ishot[i,k,:,0]],M9_arr),1))*Mo/xyz_mom
#    d_rec = np.array(np.sum(np.multiply(AA6[i,k,:,0:6],M6),1))*Mo/xyz_mom
...
#    d_rec  = np.multiply(signal.tukey(int(num_pts),0.1),d_rec)
    ...
    plt.plot(t , d_org,'k',label='forward simulation')
    plt.plot(t , d_rec,'r',label='reciprocal reconstruction')#    plt.plot(t , -d_rec,'r',label='reciprocal reconstruction')
