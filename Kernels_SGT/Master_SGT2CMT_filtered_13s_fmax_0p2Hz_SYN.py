...
def read_sgt(matrix_file_fx):
    ...
    sgtheader_9floats = np.zeros((localnp,9))    
    sgtheader_6coords = np.zeros((localnp,6))
    sgtheader_3mom = np.zeros((localnp,3))       
    ...
    for ik in range(0,localnp):
        ...
        nt0 = sgtheader_nt[ik]
        print(nt0)
#        [dt,tst,h,modelrot_sgt,modellon_sgt, modellat_sgt,  lam, mu, rho] = np.fromfile(open_file, dtype="f4", count=9)
        sgtheader_9floats[ik,:] = np.fromfile(open_file, dtype="f4", count=9)
        print(sgtheader_9floats[ik,:])
        input('-->')
#        [xsrc, ysrc, zsrc, xsta, ysta, zsta] = np.fromfile(open_file, dtype="i4", count=6) 
        sgtheader_6coords[ik,:] = np.fromfile(open_file, dtype="i4", count=6)
        
#        [xmom, ymom, zmom] = np.fromfile(open_file, dtype="f4", count=3)
        sgtheader_3mom[ik,:] = np.fromfile(open_file, dtype="f4", count=3)

        xx = np.fromfile(open_file, dtype="f4", count=nt0)
        id_max = np.argmax(np.abs(xx))
        Mxx[ik,:] = xx
        Mxx[ik,0:id_max+50] = 0
        
#        Mxx[ik,:] = np.fromfile(open_file, dtype="f4", count=nt)
        Myy[ik,:] = np.fromfile(open_file, dtype="f4", count=nt0)
        Mzz[ik,:] = np.fromfile(open_file, dtype="f4", count=nt0)         
        
        Mxy[ik,:] = np.fromfile(open_file, dtype="f4", count=nt0)
        Mxz[ik,:] = np.fromfile(open_file, dtype="f4", count=nt0)
        Myz[ik,:] = np.fromfile(open_file, dtype="f4", count=nt0) 
        #print([max(Mxx[ik,:]), max(Myy[ik,:]), max(Mzz[ik,:])])
        
    return geoproj, modellon, modellat, modelrot, xshift, yshift, globnp, localnp, nt, indx, coord_sgt, dh, \
           sgtheader_nt, sgtheader_9floats, sgtheader_6coords, sgtheader_3mom, Mxx, Myy, Mzz, Mxy, Mxz, Myz    
...
for i,statname in enumerate(statnames_RGT):
    for k in range(0,3):
        ...
        _, _, _, _, _, _, _, localnp, nt, indx, coord_sgt, dh, _, _, _, _, Mxx, Myy, Mzz, Mxy, Mxz, Myz = read_sgt(fw_file)
        print(nt)
        print(fw_file)    
        #input('-->')        
