...
def read_sgt(matrix_file_fx):
    ...
    for ik in range(0,localnp):
        ...
        xx = np.fromfile(open_file, dtype="f4", count=nt)
        id_max = np.argmax(np.abs(xx))
        #xx[id_max] = (xx[id_max-1]+xx[id_max+1])*0.5
        Mxx[ik,:] = xx
        ...
        #plt.plot(Mxx[ik,:],label=str(ik))
        plt.plot(Mxx[ik,id_max+50:nt],label=str(ik))
#    plt.plot(Mxx[1,:],label=str(ik))
        plt.ylabel('Mxx')
        plt.show()
        input('-->')    
